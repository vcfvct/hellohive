	...
	switch(action.type) {
		case 'TOGGLE_COLUMN':
			...
			if (removed) { 
				...
				if (!hasTable) { removeItem(state.tables, action.table) }
			...
			if (!hasTable) { state.tables.push(action.table) }

			return Object.assign({}, state)

		case 'TOGGLE_FILTER':
			removed = removeItem(state.filters, action.table + "." + action.column)
			if (removed) { return Object.assign({}, state) }

			state.filters.push(action.table + "." + action.column)

			hasTable = containsItem(state.tables, action.table)
			if (!hasTable) { state.tables.push(action.table) }

			let hasColumn = containsItem(state.columns, action.table + "." + action.column)
			if (!hasColumn) { state.columns.push(action.table + "." + action.column) }
...
function containsItem(array, selectedItem) {
	...
	array.map((item, index) => {
		if (item.includes(selectedItem)) { containsItem = true }
	})
